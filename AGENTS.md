...
- Game identity: use locally stored, publisher-provided official app icons for 몬길: STAR DIVE, 명조: 워더링 웨이브, 원신, and 이환 everywhere the shared game marker appears; do not fall back to generic decorative symbols while an official icon is available.
...
- Select controls: use the shared custom dark-theme select component for game, type, and status choices instead of native platform `<select>` menus so dropdown styling and keyboard behavior remain consistent across surfaces.
- Runtime data API: all platforms must discover enabled games from `/api/v1/games`, then independently exhaust each game's opaque cursor from `/api/v2/events?gameId={gameId}`. Electron caches only complete successful traversals per game; preserve another game's last successful cache when one game fails and use bundled data only when remote and cache are unavailable.
- Runtime date filtering: v2 has no date query. Selecting a date must refetch all game pages and apply overlap-aware KST filtering locally; clearing it refetches the unfiltered v2 pages.
...
- Default schedule visibility: with no exact date selected, show schedules whose KST start date falls within today and the previous six days. Enabling `전체보기` includes the full past and future history; an explicit exact-date search uses overlap-aware matching for that date.
- Undated pickups: banner events with `startsAt: null` remain available in a dedicated `시간 미정` timeline group regardless of exact-date filtering or default-history visibility. Keep this group collapsed by default and reveal its rows when the user activates the group header; other undated event types remain excluded, and undated pickups do not appear in calendar date cells.
