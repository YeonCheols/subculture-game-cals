...
GameTime is a Korean-first responsive web and Electron desktop calendar for official schedules from:
...
`src/App.jsx` currently owns data loading, normalization for display, filters, subscriptions, reminder state, timeline/calendar rendering, and the shared detail modal. `src/data/schedules.js` defines game identity and locally stored official icons; it is not the runtime schedule source.
...
- Game subscription and reminder selections persist in `localStorage`.
