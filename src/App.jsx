...
export function App() {
  ...
  const refresh = useCallback(async () => { setIsRefreshing(true); try { const [payload, allPayload] = dateFilter ? await Promise.all([fetchScheduleData(dateFilter), fetchScheduleData("")]) : [await fetchScheduleData(""), null]; if (!Array.isArray(payload.events) || (allPayload && !Array.isArray(allPayload.events))) throw new Error("events response is not an array"); const discoveredGames = applyGameCatalog(payload.games || allPayload?.games); const discoveredIds = discoveredGames.map((game) => game.id); const newIds = discoveredIds.filter((id) => !knownGameIds.includes(id)); const undatedPickups = (allPayload?.events || []).filter((event) => event.type === "banner" && !event.startsAt); const mergedEvents = [...payload.events, ...undatedPickups.filter((pickup) => !payload.events.some((event) => event.id === pickup.id))]; if (newIds.length) setSubscribed((current) => [...new Set([...current, ...newIds])]); setKnownGameIds((current) => [...new Set([...current, ...discoveredIds])]); setReminderEvents(allPayload?.events || payload.events); setRemoteGroups(toScheduleGroups(mergedEvents, discoveredGames)); const status = payload.status || allPayload?.status; if (status?.retrievedAt) setLastSyncedAt(status.retrievedAt); } catch (error) { console.error("공식 일정 API를 불러오지 못했습니다.", error); } finally { setIsRefreshing(false); } }, [dateFilter, knownGameIds, setKnownGameIds, setSubscribed]);
