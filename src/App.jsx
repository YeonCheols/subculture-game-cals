...
const localGamePresentation = new Map(games.map((game) => [game.id, game]));

function applyGameCatalog(catalogGames) {
  if (!Array.isArray(catalogGames)) return games;
  const presented = catalogGames.map((game) => { const local = localGamePresentation.get(game.id); return { ...game, color: local?.color || "#35b6ff", soft: local?.soft || "#102b3d", icon: local?.icon || game.icon?.url || "" }; });
  games.splice(0, games.length, ...presented);
  return games;
}
...
function MobileWeekStrip({ dateFilter, onChange }) {
  const selectedDate = dateFilter || kstDateKey();
  ...
  return <div className="mobile-week-strip" role="group" aria-label="일정 날짜 선택">{Array.from({ length: 7 }, (_, index) => { const date = new Date(start); date.setDate(start.getDate() + index); const key = kstDateKey(date); const isSelected = key === selectedDate; return <button key={key} type="button" className={isSelected ? "is-selected" : ""} onClick={() => onChange(isSelected ? "" : key)}><span>{weekday.format(date).replace("요일", "")}</span><strong>{new Intl.DateTimeFormat("ko-KR", { timeZone: "Asia/Seoul", day: "numeric" }).format(date).replace("일", "")}</strong>{isSelected && <i />}</button>; })}</div>;
}

function MobileScheduleHero({ item, notifications, toggleNotification, onOpen }) {
  if (!item) return <section className="mobile-next-event mobile-next-event--empty"><span>다음 일정</span><h1>예정된 일정이 없습니다</h1><p>새 일정이 확인되면 이곳에 표시됩니다.</p></section>;
  const game = games.find((entry) => entry.id === item.gameId);
  const notified = notifications.includes(item.id);
  return <section className="mobile-next-event"><span>다음 일정</span><div className="mobile-next-event__content" role="button" tabIndex={0} onClick={() => onOpen(item)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onOpen(item); }}><time>{item.time}</time><GameMark game={game} size="md" /><div><b className={`type-badge type-badge--${item.typeKey}`}>{item.type}</b><h1>{item.title}</h1></div></div><button type="button" className={`mobile-next-event__notify ${notified ? "is-active" : ""}`} onClick={() => toggleNotification(item.id)}><IconBellFilled size={19} />{notified ? "알림 설정됨" : "알림 받기"}</button></section>;
}

function MobileAgenda({ items, notifications, toggleNotification, onOpen }) {
  if (!items.length) return <div className="empty-state mobile-empty-state"><IconCalendar size={30} /><strong>선택한 날짜에 일정이 없습니다</strong><span>다른 날짜를 선택해 보세요.</span></div>;
  return <div className="mobile-agenda">{items.slice(0, 3).map((item) => { const game = games.find((entry) => entry.id === item.gameId); const notified = notifications.includes(item.id); return <article key={item.id} className="mobile-agenda__row" role="button" tabIndex={0} onClick={() => onOpen(item)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onOpen(item); }}><time>{item.time}</time><GameMark game={game} size="sm" /><div><b className={`type-badge type-badge--${item.typeKey}`}>{item.type}</b><strong>{item.title}</strong><small>{game?.shortName} · {item.status}</small></div><button className={notified ? "is-active" : ""} aria-label={`${item.title} 알림`} onClick={(event) => { event.stopPropagation(); toggleNotification(item.id); }}>{notified ? <IconBellFilled size={18} /> : <IconBell size={18} />}</button><IconChevronRight size={18} /></article>; })}</div>;
}

function MobileSurface({ page, setPage, query, setQuery, dateFilter, setDateFilter, typeFilter, setTypeFilter, statusFilter, setStatusFilter, gameFilter, setGameFilter, visibleGroups, allItems, notifications, toggleNotification, selectedItem, setSelectedItem, reminderItems }) {
  const [filtersOpen, setFiltersOpen] = useState(false);
  const mobileItems = visibleGroups.flatMap((group) => group.items).filter((item) => item.startsAt).sort((a, b) => Date.parse(a.startsAt) - Date.parse(b.startsAt));
  const nextItem = mobileItems.find((item) => Date.parse(item.startsAt) >= Date.now()) || mobileItems[0];
  const agendaItems = mobileItems.filter((item) => !nextItem || item.id !== nextItem.id);
  const navItems = [["schedule", "일정", IconCalendar], ["calendar", "캘린더", IconCalendar], ["notifications", "알림", IconBell], ["redemption", "리딤코드", IconGift]];
  return <div className="mobile-shell"><header className="mobile-topbar"><div><span>GAME TIME</span><strong>게임타임</strong></div><label><IconSearch size={20} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="일정 검색" aria-label="일정 검색" /></label><button type="button" onClick={() => setPage("notifications")} aria-label="알림"><IconBell size={22} />{notifications.length > 0 && <b>{notifications.length}</b>}</button></header><main className="mobile-content">{page === "schedule" && <><MobileScheduleHero item={nextItem} notifications={notifications} toggleNotification={toggleNotification} onOpen={setSelectedItem} /><MobileWeekStrip dateFilter={dateFilter} onChange={setDateFilter} /><section className="mobile-upcoming"><header><div><span>다가오는 일정</span><h2>{dateFilter ? `${dateFilter.replaceAll("-", ".")} 일정` : "다음에 시작해요"}</h2></div><button type="button" onClick={() => setFiltersOpen((current) => !current)} aria-expanded={filtersOpen}><IconAdjustmentsHorizontal size={18} />필터</button></header>{filtersOpen && <div className="mobile-filter-drawer"><FilterSelect value={gameFilter} options={["모든 게임", ...games.map((game) => game.shortName)]} onChange={setGameFilter} /><FilterSelect value={typeFilter} options={typeLabels} onChange={setTypeFilter} /><FilterSelect value={statusFilter} options={statusLabels} onChange={setStatusFilter} /></div>}<MobileAgenda items={agendaItems} notifications={notifications} toggleNotification={toggleNotification} onOpen={setSelectedItem} /><button type="button" className="mobile-view-all" onClick={() => { setDateFilter(""); setPage("calendar"); }}>전체 일정 보기<IconChevronRight size={19} /></button></section></>}{page === "calendar" && <MiniCalendar groups={visibleGroups} onOpen={setSelectedItem} focusDate={dateFilter} />}{page === "notifications" && <NotificationPanel embedded allItems={reminderItems} notifications={notifications} toggleNotification={toggleNotification} />}{page === "redemption" && <RedemptionCodes searchQuery={query} />}</main><nav className="mobile-bottom-nav" aria-label="모바일 주 메뉴">{navItems.map(([id, label, Icon]) => <button type="button" className={page === id ? "is-active" : ""} key={id} onClick={() => setPage(id)}><Icon size={22} /><span>{label}</span>{id === "notifications" && notifications.length > 0 && <b>{notifications.length}</b>}</button>)}</nav><EventDetailModal item={selectedItem} onClose={() => setSelectedItem(null)} notifications={notifications} toggleNotification={toggleNotification} /></div>;
}

export function App({ surface = "desktop-web" }) {
  ...
  const [knownGameIds, setKnownGameIds] = usePersistentState("gametime:known-games", games.map((game) => game.id));
  ...
  const visibleGroups = useMemo(() => { const today = kstDateKey(); const recentStart = kstDateKey(new Date(Date.now() - 6 * 24 * 60 * 60 * 1000)); return activeGroups.map((group) => ({ ...group, items: group.items.filter((item) => { const game = games.find((entry) => entry.id === item.gameId); return subscribed.includes(item.gameId) && (gameFilter === "모든 게임" || game.shortName === gameFilter) && (typeFilter === "전체" || item.type === typeFilter) && (statusFilter === "전체 상태" || item.status === statusFilter) && (group.date === UNDATED_PICKUP_GROUP || showAll || dateFilter || (group.date >= recentStart && group.date <= today)) && (`${item.title} ${game.name}`.toLowerCase().includes(query.toLowerCase())); }) })).filter((group) => group.items.length); }, [activeGroups, query, gameFilter, dateFilter, typeFilter, statusFilter, showAll, subscribed]);
  ...
  const refresh = useCallback(async () => { setIsRefreshing(true); try { const [payload, allPayload] = dateFilter ? await Promise.all([fetchScheduleData(dateFilter), fetchScheduleData("")]) : [await fetchScheduleData(""), null]; if (!Array.isArray(payload.events) || (allPayload && !Array.isArray(allPayload.events))) throw new Error("events response is not an array"); const discoveredGames = applyGameCatalog(payload.games || allPayload?.games); const discoveredIds = discoveredGames.map((game) => game.id); const undatedPickups = (allPayload?.events || []).filter((event) => event.type === "banner" && !event.startsAt); const mergedEvents = [...payload.events, ...undatedPickups.filter((pickup) => !payload.events.some((event) => event.id === pickup.id))]; setSubscribed((current) => { const next = [...new Set([...current, ...discoveredIds])]; return next.length === current.length ? current : next; }); setKnownGameIds((current) => { const next = [...new Set([...current, ...discoveredIds])]; return next.length === current.length ? current : next; }); setReminderEvents(allPayload?.events || payload.events); setRemoteGroups(toScheduleGroups(mergedEvents, discoveredGames)); const status = payload.status || allPayload?.status; if (status?.retrievedAt) setLastSyncedAt(status.retrievedAt); } catch (error) { console.error("공식 일정 API를 불러오지 못했습니다.", error); } finally { setIsRefreshing(false); } }, [dateFilter, setKnownGameIds, setSubscribed]);
  useEffect(() => { refresh(); const removePlatformListener = onPlatformScheduleRefresh(refresh); const refreshWhenVisible = () => { if (document.visibilityState === "visible") refresh(); }; document.addEventListener("visibilitychange", refreshWhenVisible); return () => { removePlatformListener(); document.removeEventListener("visibilitychange", refreshWhenVisible); }; }, [refresh]);
