export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET" && ["/remote-api/events", "/remote-api/collection-status"].includes(url.pathname)) {
      ...
      return new Response(response.body, { status: response.status, headers: { "content-type": "application/json; charset=utf-8", "cache-control": endpoint === "events" ? "public, max-age=300" : "public, max-age=60" } });
    }
